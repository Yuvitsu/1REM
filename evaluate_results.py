...
class ModelResultEvaluator:
    def __init__(self, model_name, predictions_path, true_values_path, output_dir_base="test_results/E420"):
        """
        モデルの予測結果を評価するクラス。
        ...
        self.output_dir = os.path.join(output_dir_base, f"{model_name}_heatmaps")
    ...
    def load_data(self):
        """NumPy ファイルをロード"""
        print(f"=== {self.model_name}: データのロード ===")
        self.test_predictions = np.load(self.predictions_path)  # (サンプル数, 6)
        self.test_true_values = np.load(self.true_values_path)  # (サンプル数, 6)
        print(f"✅ {self.model_name} test_predictions.shape: {self.test_predictions.shape}")
        print(f"✅ {self.model_name} test_true_values.shape: {self.test_true_values.shape}")

    def apply_interpolation(self):
        """スプライン補間を適用"""
        print(f"=== {self.model_name}: スプライン補間を適用 ===")
    ...
    def compute_mse(self):
        ...
        print(f"✅ {self.model_name} MSE 計算完了: {self.mse_value}")
    ...
    def save_heatmaps(self, data, title_prefix, filename_prefix):
        ...
        for i in range(min(5, len(data))):  # 最初の5枚を保存
            ...
            plt.colorbar()
            plt.title(f"{self.model_name}: {title_prefix} {i+1}")
            plt.savefig(os.path.join(self.output_dir, f"{filename_prefix}_{i+1}.png"))
            plt.close()
        print(f"✅ {self.model_name}: {title_prefix} のヒートマップを保存しました。")

    def save_all_heatmaps(self):
        """補間後の予測値、真値、差分のヒートマップを保存"""
        print(f"=== {self.model_name}: ヒートマップを保存 ===")
    ...
    def evaluate(self):
        ...
        print(f"✅ {self.model_name}: ヒートマップと MSE の結果を {self.output_dir} に保存しました。")

# --- メイン処理 ---
if __name__ == "__main__":
    # LSTM の評価
    lstm_evaluator = ModelResultEvaluator(
        model_name="LSTM",
        predictions_path="test_results/E420/LSTM_test_results/test_predictions.npy",
        true_values_path="test_results/E420/LSTM_test_results/test_true_values.npy"
    )
    lstm_evaluator.evaluate()

    # Transformer の評価
    transformer_evaluator = ModelResultEvaluator(
        model_name="Transformer",
        predictions_path="test_results/E420/Transformer_test_results/test_predictions.npy",
        true_values_path="test_results/E420/Transformer_test_results/test_true_values.npy"
    )
    transformer_evaluator.evaluate()
